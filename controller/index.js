...
var Controller = function( Unhcr, BaseController ){
  ...
  controller.index = function(req, res){
    res.send('This is the UNHCR provider. ');
  };
  ...
  controller.register = function(req, res){
    if ( !req.body.host ){
      ...
      Unhcr.register( req.body.id, req.body.host, function(err, id){
        if (err) {
   ...
   controller.getData = function(req,res){
	   Unhcr.find(req.params.id, req.query, function(err, data){
	   if ( err ){
          res.json( err, 500 );
		  return
	   }
